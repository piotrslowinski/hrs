...
public class EmployeeTest {
    ...
    @Test
    public void shouldAllowMultipleChangeOfSalary(){
        //when
        sut.changeSalary(SALARY);
    ...
    public void shouldKeepSalaryHistory(){
        ...
        assertEquals(Arrays.asList(t0,t1,t2),
                history.stream().map((s) -> s.getFromDate()).collect(Collectors.toList())
        );
        assertEquals(Arrays.asList(t1,t2,TimeProvider.MAX_DATE),
    ...
    public void shouldFireEmployee(){
        ...
        sut.assignDepartment(d1);
        sut.changeTitle("CEO");
        sut.changeSalary(50000);
        timeMachine.travel(Duration.ofDays(100));
        sut.assignDepartment(d2);

        //when
        timeMachine.travel(Duration.ofDays(265));
        LocalDate t2 = timeMachine.today();
        sut.fire(sut);

        //then
        Collection<DepartmentAssignment> history = sut.getDepartmentsHistory();
        Collection<Title> titles = sut.getTitleHistory();
        Collection<Salary> salaries = sut.getSalaries();
        LinkedList<Salary> s = new LinkedList<>(salaries);


        assertEquals(t2, s.getLast().getToDate());
        assertEquals(Arrays.asList(t2),titles.stream().map(Title::getToDate).collect(Collectors.toList()));
        assertEquals(Arrays.asList(t2,t2), history.stream().map(DepartmentAssignment::getToDate).collect(Collectors.toList()));
    }
    ...
    public int getCurrentSalaryValue() {
        return getCurrentSalary().get().getValue();
    }

    public String getCurrentTitleName() {
        return sut.getCurrentTitle().get().getName();
    }
}
